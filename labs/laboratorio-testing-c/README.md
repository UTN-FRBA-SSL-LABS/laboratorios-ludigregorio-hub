...
> R:La función carrito_init se encarga de inicializar el carrito de compras estableciendo su contador de elementos en cero
...
> R:falla , el valor que esperaba es 700, el valor que obtuvo es 350
...
> R:Debería multiplicar el precio del producto por su cantidad antes de sumarlo al acumulador
...
> R:falla, devuelve supera MAX_ITEMS
...
> R:Si ejecutaste todos los tests anteriores (test_carrito_nuevo, test_agregar_uno, test_carrito_lleno, test_compra_con_descuento, test_agregar_hasta_llenar), la respuesta debería ser que no hay ninguna línea con #####. Todos los caminos posibles del código (el if verdadero, el if falso con el return 0, el bucle for, etc.) fueron ejercitados.
...
> R:Un test unitario aísla y prueba una única función o componente en particular (por ejemplo, verificar que carrito_contar devuelve 0 en un carrito vacío). Un test de integración prueba cómo interactúan múltiples componentes juntos (por ejemplo, usar carrito_agregar varias veces y luego llamar a carrito_total para ver si el resultado combinado es correcto). El bug de carrito_total fue detectado por un test de integración, ya que requería que previamente se agregaran productos con cantidades específicas usando carrito_agregar para luego evaluar el total conjunto.
...
> R:C confía en el programador y permite acceso directo a la memoria a través de punteros y arreglos sin verificar los límites en tiempo de ejecución. Escribir fuera de un arreglo  puede corromper datos, causar segmentation faults o abrir vulnerabilidades de seguridad graves. En lenguajes como pyton y java de ejecución o el compilador controlan el acceso a la memoria; si intentás acceder a un índice inválido, el programa aborta de forma segura lanzando una excepción (ej. IndexOutOfBoundsException) antes de corromper la memoria.
...
> R:Mirar el código directamemte es fácil pasar por alto errores lógicos, especialmente en sistemas complejo. Los tests automatizados proporcionan una verificación empírica, repetible y rápida. Además, actúan como un seguro contra regresiones: si en el futuro modificás el código, podés correr los tests en segundos para garantizar que no rompiste una funcionalidad que antes andaba bien.
...
> R:: Porque el número 1 es el elemento neutro de la multiplicación. El bug omitía multiplicar el precio por la cantidad (total += precio). Al ser la cantidad igual a 1, matemáticamente precio y precio * 1 dan exactamente el mismo resultado, enmascarando el error lógico del código. El bug solo se hace visible cuando cantidad > 1.
